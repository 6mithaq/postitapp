...
export default function CruisesPage() {
  const [filteredCruises, setFilteredCruises] = useState<Cruise[] | null>(null);
  
  const { data: cruises, isLoading, isError } = useQuery<Cruise[]>({
    queryKey: ["/api/cruises"],
  });

  // Handle search/filter results
  const handleSearchResults = (results: Cruise[]) => {
